        ...
        <div class="space-y-6 lg:col-span-2">
            ...
            <x-filament::section icon="heroicon-o-video-camera" heading="Jadwal wawancara">
                ...
                <x-filament-panels::form id="form"
                    :wire:key="$this->getId() . '.forms.' . $this->getFormStatePath()" wire:submit="saveAppointment">
                    @if (!$appointed)
                        <div class="flex gap-6">
                            ...
                            {{-- <x-filament::input type="datetime-local" wire:model="appointment_at"
                                    min="{{ now()->format('Y-m-d') . 'T09:00' }}"
                                    max="{{ now()->addWeek()->format('Y-m-d') . 'T17:00' }}" required /> --}}
                        ...
                        {{-- <div class="flex gap-6">
                            <x-filament::input.wrapper class="w-full">
                                <x-filament::input type="date" wire:model="appointment_date"
                                    min="{{ now()->format('Y-m-d') }}" max="{{ now()->addWeek()->format('Y-m-d') }}"
                                    required />
                            </x-filament::input.wrapper>

                            <x-filament::input.wrapper class="w-full">
                                <x-filament::input type="time" wire:model="appointment_time" min="09:00"
                                    max="17:00" required />
                            </x-filament::input.wrapper>
            ...
            <x-filament::section compact
                ...
                Untuk konfirmasi pembayaran atau informasi lainnya, silakan chat melalui <a
                    href="https://wa.me/6282329528670" target="_blank"
                    class="font-semibold underline decoration-dotted text-primary-500">WhatsApp ini</a>
            </x-filament::section>
            ...
            <x-filament::section icon="heroicon-o-clock" class="overflow-y-auto">
                ...
                <ol
                    class="flex flex-col flex-wrap gap-4 text-sm font-medium text-center text-gray-500 sm:items-center sm:flex-row xl:justify-between dark:text-gray-400 sm:text-base">
                ...
                <div class="space-y-4">
                    @foreach ($completeness as $completion)
                        <a href="{{ $completion['url'] ?? 'javascript:;' }}" class="block group">
                            <x-filament::section compact
                                class="transition-colors dark:group-hover:bg-gray-800 group-hover:bg-gray-50">
