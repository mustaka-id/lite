...
<x-filament-panels::page class="fi-dashboard-page">
    <div class="grid gap-6 lg:grid-cols-3">
            ...
            <x-filament::section icon="heroicon-o-pencil-square">
                ...
                <div class="space-y-4">
                    <x-filament::section compact
                        class="transition-colors dark:group-hover:bg-gray-800 group-hover:bg-gray-50" collapsible
                        collapsed>
                        <x-slot name="heading">
                            <div class="flex items-center justify-between gap-4">
                                <div @class([
                                    'grid rounded-full place-items-center size-8 text-sm font-semibold',
                                    $this->registrant->registered_at
                        ...
                        <div class="space-y-2">
                            @foreach ($this->getRegistrationFormSection() as $section)
                                <a href="{{ $section['url'] ?? 'javascript:;' }}" class="block group">
                                    <div
                                        class="px-2 py-2 transition-colors rounded-lg dark:group-hover:bg-gray-800 group-hover:bg-gray-50">
                                        <div class="flex items-center justify-between gap-4">
                                            <div @class([
                                                'grid rounded-full place-items-center size-8 text-sm font-semibold',
                                                $section['value']
                            ...
                            @if (!$this->registrant->registered_at)
                                <div class="p-2 mt-2 space-y-2">
                                    <hr>
                                    <p>Apabila Anda sudah yakin dengan formulir pendaftaran Anda, silakan klik tombol
                                        "Kirim Formulir" di bawah ini</p>
                                    <x-filament::button wire:click="finalizeRegistration"
                                        icon="heroicon-o-paper-airplane" :disabled="$completeness_precentage < 100 || $this->registrant->registered_at">
                                        Kirim Formulir
                                    </x-filament::button>
                                </div>
                            @endif
                            ...
                            <x-filament-panels::form id="form"
                                ...
                                @else
                                    <p>Anda telah menjadwalkan wawancara pada
                                        ...
                                        jika ada perubahan silakan hubungi Petugas PPDB melalui WhatsApp
                                    </p>
                                @endif
                            </x-filament-panels::form>
                        @else
                            <p class="text-red-600">Mohon untuk selesaikan dahulu tahap pengisian formulir pendaftaran
                            </p>
                        @endif
                    </x-filament::section>
                    <x-filament::section compact
                        ...
                        @if (!$accepted)
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p>Anda telah diterima sebagai santri Madrasah Aliyah Ihsaniyya Yogyakarta per
                                        {{ $this->registrant->accepted_at?->isoFormat('LLLL') }}</p>
                                </div>
                            </div>
                        @else
                            <p>Belum ada pengumuman, pastikan Anda telah melakukan sesi wawancara dengan petugas PPDB
                            </p>
                        @endif
                    </x-filament::section>
                    <x-filament::section compact
                        ...
                        @if ($remain > 0)
                            <div class="space-y-2">
                                ...
                                <x-filament::section compact
                                    class="ring-danger-200 dark:ring-danger-800/60 bg-danger-50 dark:bg-danger-900/40 text-danger-700 dark:text-danger-400">
                                    Silakan lakukan pembayaran ke Nomor Rekening: BRI
                                    <strong>2200-01-000314-30-8</strong>
                                    a.n. Ponpes Nurul Ihsan YK.
                                </x-filament::section>
                            </div>
                        @else
        ...
        <div class="space-y-6">
            <x-filament::section
                class="ring-primary-200 dark:ring-primary-800/60 bg-primary-50 dark:bg-primary-900/40 text-primary-700 dark:text-primary-400"
                icon="heroicon-o-information-circle">
                <x-slot name="heading">
                    Informasi
                </x-slot>
                Untuk konfirmasi pembayaran atau informasi lainnya, silakan chat melalui <a
                    href="https://wa.me/6282329528670?text=Assalamu%27alaikum%20admin%20MASAYA%2C%20saya%20{{ $this->registrant->user->name }}%0A%0ASaya%20ingin%20konfirmasi%20pembayaran%2Fmenanyakan%20informasi%20tentang%20%20pendaftaran%20santri%20baru%20Madrasah%20Aliyah%20Ihsaniyya.%20%0A%0ATerimakasih"
                    target="_blank" class="font-semibold underline decoration-dotted text-primary-500">WhatsApp ini</a>
            </x-filament::section>
